...
Android version of TDP. The randomization was conduct at the municipal level.
We randomly selected `r as.numeric(table(impact_evaluation_phase1$treat)[1])`
municipalities to the control group and included `r
format(as.numeric(table(impact_evaluation_phase1$treat)[2]), big.mark = ",")`
in the treatment group. Our control condition consists in removing all
information about school construction from the TDP app in the chosen
municipalities, so that citizens were unable to report constructions in the
